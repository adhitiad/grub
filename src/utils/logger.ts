...
class Logger {
  ...
  private log(level: LogLevel, message: string, data?: any, error?: Error, userId?: string, requestId?: string) {
  ...
  error(message: string, error?: Error, data?: any, userId?: string, requestId?: string) {
  ...
  authSuccess(userId: string, action: string, requestId?: string) {
    this.info(`Authentication successful: ${action}`, { userId }, userId, requestId);
  }

  authFailure(action: string, reason: string, requestId?: string) {
    this.warn(`Authentication failed: ${action}`, { reason }, undefined, requestId);
  }
  ...
  apiResponse(method: string, path: string, statusCode: number, duration: number, userId?: string, requestId?: string) {
    this.info(`API Response: ${method} ${path} - ${statusCode} (${duration}ms)`, undefined, userId, requestId);
  }

  databaseOperation(operation: string, collection: string, success: boolean, userId?: string, requestId?: string) {
  ...
  paymentOperation(operation: string, orderId: string, amount?: number, success?: boolean, userId?: string, requestId?: string) {
    const level = success === false ? LogLevel.ERROR : LogLevel.INFO;
    const message = `Payment ${operation} for order ${orderId}${amount ? ` (${amount})` : ''}`;
    this.log(level, message, { orderId, amount, success }, undefined, userId, requestId);
  }
}
...
export const createErrorResponse = (message: string, error?: string, details?: any) => ({
  success: false,
  message,
  error,
  details,
